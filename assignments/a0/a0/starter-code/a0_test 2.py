...
class  TestReadResult(unittest.TestCase):
    def setUp(self):
        self._io = "header\n"+','.join(['35090', '"ez4"', '"St. Paul\'s"',
                                '" 1"', '"Toronto"', 'N', 'N', '""', '1',
                                '367', '"Bennett"', '""', '"Carolyn"',
                                '"Liberal"', '"Liberal"', 'Y', 'Y', '113'])
        self._io_copy = self._io[:]
    def test_basic(self):
        a = Election(date(1989,12,21))
        a.read_results(StringIO(self._io_copy))
        exp_riding = ["ez4"]
        exp_party = ["Liberal"]
        exp_res = {"ez4":{"Liberal":113}}
        act_riding = a._ridings
        act_party = a._parties
        act_res = a._results
        self.assertCountEqual(exp_riding,act_riding)
        self.assertCountEqual(exp_party, act_party)
        self.assertDictEqual(exp_res, act_res)
    ...
    def test_update_2(self):
        self._io_copy = self._io + "\n" + ','.join(['35090', '"ez5"', '"St. Paul\'s"',
                                '" 1"', '"Toronto"', 'N', 'N', '""', '1',
                                '367', '"Bennett"', '""', '"Carolyn"',
                                '"pc"', '"Liberal"', 'Y', 'Y', '113'])
        a = Election(date(2011, 11, 21))
        a.read_results(StringIO(self._io_copy))
        exp_riding = ['ez4', 'ez5']
        exp_party = ["pc", "Liberal"]
        exp_res = {"ez4":{"Liberal":113}, "ez5":{"pc":113}}
        act_riding = a._ridings
        act_party = a._parties
        act_res = a._results
        self.assertCountEqual(exp_riding, act_riding)
    ...
    def test_update_3(self):
        ...
        exp_riding = ['ez4']
        exp_party = ["pc", "Liberal"]
        exp_res = {"ez4":{"Liberal":113, "pc":113}}
        act_riding = a._ridings
        act_party = a._parties
        act_res = a._results
        self.assertCountEqual(exp_riding, act_riding)
    ...
    def test_update_4(self):
        ...
        exp_riding = ['ez4', 'ez5']
        exp_party = ["pc", "Liberal"]
        exp_res = {"ez4":{"Liberal":226}, "ez5":{"pc":113}}
        act_riding = a._ridings
        act_party = a._parties
        act_res = a._results
        self.assertCountEqual(exp_riding, act_riding)
...
class TestJuReadResult(unittest.TestCase):
    def setUp(self):
        self._io = "header\n"+','.join(['35090', '"ez4"', '"St. Paul\'s"',
                                '" 1"', '"Toronto"', 'N', 'N', '""', '1',
                                '367', '"Bennett"', '""', '"Carolyn"',
    ...
    def test_basic(self):
        a = Jurisdiction("EZ")
        a.read_results(2012, 12, 20, StringIO(self._io))
        exp_ele = Election(date(2012, 12, 20))
        exp_ele._results = {"ez4":{"Liberal":113}}
        exp_date = date(2012, 12, 20)
        exp_res = exp_ele._results
        assert len(list(a._history.keys())) == 1
        act_date = list(a._history.keys())[0]
        act_res = a._history[act_date]._results
        self.assertEqual(exp_date, act_date)
        self.assertDictEqual(exp_res, act_res)

    def test_update_1(self):
        a = Jurisdiction("EZ")
        b = Election(date(2018,11,20))
        b.read_results(StringIO(self._io))
        a._history[date(2017, 11, 20)] = b
        a.read_results(2017, 11, 20, StringIO(self._io))
        exp_res = {"ez4":{"Liberal":226}}
        exp_date = date(2017, 11, 20)
        assert len(list(a._history.keys())) == 1
        act_date = list(a._history.keys())[0]
        act_res = a._history[act_date]._results
        self.assertEqual(exp_date, act_date)
    ...
    def test_update_2(self):
        ...
        a = Jurisdiction("EZ")
        a.read_results(2017, 11, 20, StringIO(self._io))
        a.read_results(2018, 11, 20, StringIO(temp))
        exp_date = [date(2017, 11, 20), date(2018, 11, 20)]
        act_date = sorted(list(a._history.keys()))
        assert len(list(a._history.keys())) == 2
        self.assertEqual(exp_date,act_date)
        exp_res = [{"ez4":{"Liberal":113}}, {'ez5':{"pc":113}}]
        act_res = [a._history[date]._results for date in act_date]
        self.assertEqual(exp_date, act_date)
        self.assertCountEqual(exp_res, act_res)
    ...
    def test_update_3(self):
        a = Jurisdiction("EZ")
        a.read_results(2017, 11, 20, StringIO(self._io))
        a.read_results(2018, 11, 20, StringIO(self._io))
        exp_date = [date(2017, 11, 20), date(2018, 11, 20)]
        act_date = sorted(list(a._history.keys()))
        assert len(list(a._history.keys())) == 2
        self.assertEqual(exp_date,act_date)
        exp_res = [{"ez4":{"Liberal":113}}, {"ez4":{"Liberal":113}}]
        act_res = [a._history[date]._results for date in act_date]
        self.assertEqual(exp_date, act_date)
        self.assertCountEqual(exp_res, act_res)
