...
class Election:
    ...
    def read_results(self, instream: IO[str]) -> None:
        ...
        for line in instream:
            while line != '':
                ...
                self.update_results(str(content[1]), str(content[13]), int(content[17]))
    ...
    def riding_winners(self, riding: str) -> List[str]:
        ...
        result = []
        vote_num = 0
        for b in self._results[riding]:
            if self._results[riding][b] > vote_num:
                vote_num = self._results[riding][b]
                result = [b]
            elif self._results[riding][b] == vote_num:
                result.append(b)
    ...
    def popular_vote(self) -> Dict[str, int]:
        ...
        for c in self._results:
            for d in self._results[c]:
                if d in result_pop:
                    result_pop[d] += self._results[c][d]
                else:
                    result_pop[d] = self._results[c][d]
        return result_pop
    ...
    def party_seats(self) -> Dict[str, int]:
        ...
        for e in self._results:
            for f in self._results[e]:
                if f not in result_par:
                    result_par[f] = 0
            win_par = self.riding_winners(e)
            if len(win_par) == 1:
                result_par[win_par[0]] += 1
        return result_par
...
class Jurisdiction:
    ...
    def read_results(self, year: int, month: int, day: int, instream: IO[str]) \
        ...
        if date_r not in self._history:
            self._history[date_r] = Election(date_r)

        self._history[date_r].read_results(instream)
    ...
    def party_wins(self, party: str) -> List[date]:
        ...
        for i in self._history:
            winners = self._history[i].election_winners()
            for j in winners:
                if j in result_win:
                    result_win[j].append(i)
                else:
                    result_win[j] = [i]
        if party not in result_win:
            result_win[party] = []
        return result_win[party]
    ...
    def party_history(self, party: str) -> Dict[date, float]:
        ...
        for k in self._history:
            vote_result = self._history[k].popular_vote()
            total_vote = 0
            if party in vote_result:
                for l in vote_result:
                    total_vote += vote_result[l]
                for o in vote_result:
                    if o not in result_his:
                        result_his[o] = {k: round(vote_result[o] / total_vote, 2)}
                    else:
                        result_his[o][k] = round(vote_result[o] / total_vote, 2)
            else:
                if party not in result_his:
                    result_his[party] = {k: 0.0}
                else:
                    result_his[party][k] = 0.0
        return result_his[party]
    ...
    def riding_changes(self) -> List[Tuple[Set[str], Set[str]]]:
        ...
        for p in self._history:
            every_ridings[p] = set(self._history[p].ridings_of())
            all_keys.append(p)

        if len(all_keys) == 1:
            return []
        else:
            for q in range(len(all_keys) - 1):
                removed = every_ridings[all_keys[q]] - every_ridings[all_keys[q + 1]]
                added = every_ridings[all_keys[q + 1]] - every_ridings[all_keys[q]]
                #if removed == set():
                    #removed = {}
                #if added == set():
                    #added = {}
                result_change.append((removed, added))
