...
def mergesort3(lst: List) -> List:
    ...
    if len(lst) < 2:
        ...
        num = len(lst) // 3
        first = mergesort3(lst[:num])
        second = mergesort3(lst[num:num*2])
        third = mergesort3(lst[num*2:])

        return merge3(first, second, third)
...
def merge3(lst1: List, lst2: List, lst3: List) -> List:
    ...
    while index1 < len(lst1) and index2 < len(lst2) and index3 < len(lst3):
        if lst1[index1] <= lst2[index2] and lst1[index1] <= lst3[index3]:
            ...
            lst.append(lst3[index3])
            index3 += 1
    if index1 == len(lst1):
        lst.extend(_first_two(lst2, index2, lst3, index3))
    elif index2 == len(lst2):
        lst.extend(_first_two(lst1, index1, lst3, index3))
    else:
        lst.extend(_first_two(lst1, index1, lst2, index2))

    return lst


def _first_two(lst1: List, index1: int, lst2: List, index2: int) -> List:
    """Sorted the two list
    """
    hh = []
    while index1 < len(lst1) and index2 < len(lst2):
        if lst1[index1] <= lst2[index2]:
            hh.append(lst1[index1])
            index1 += 1
        else:
            hh.append(lst2[index2])
            index2 += 1
    return hh +  lst1[index1:] + lst2[index2:]
