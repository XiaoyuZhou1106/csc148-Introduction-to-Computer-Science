...
class BoolOp(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        False
        """
        result = self.values[0].evaluate()
        if self.op == 'and':
            for b in self.values:
                result = result and b.evaluate()
        else:
            for c in self.values:
                result = result or c.evaluate()
        return result
...
class Compare(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        True
        """
        first = self.comparisons[0][0]
        result = None
        if first == '<=':
            result = self.left.evaluate() <= self.comparisons[0][1].evaluate()
        if first == '<':
            result = self.left.evaluate() < self.comparisons[0][1].evaluate()

        if len(self.comparisons) > 1:
            for i in range(1, len(self.comparisons)):
                if self.comparisons[i][0] == '<=':
                    result = result and (self.comparisons[i - 1][1].evaluate() \
